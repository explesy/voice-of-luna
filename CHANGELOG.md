...
### Fixed
- Список голосов снова скрывает неиспользуемые языки, даже если выбран иностранный голос; выбранный вариант остаётся видимым отдельной строкой.
- Каждый голос теперь показывает свой точный языковой тег (например, `ru-RU`, `es-ES`), а основная Edge-группа содержит только русские голоса.

## [0.29.2] - 2026-09-09

### Changed
- Зафиксирован новый локальный TTS benchmark без Edge и без Codex-вызовов; результаты и ограничения добавлены в канонический документ производительности.
...
### Fixed
- Remove the active plugin panel reliably when switching back to the neutral Core plugin, including the `panel: null` response path.
